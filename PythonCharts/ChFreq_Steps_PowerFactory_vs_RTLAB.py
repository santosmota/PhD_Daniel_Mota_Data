#################################################################
# Frequency / FCR Chapter
# Generates figure with validation of Powerfactory vs PHIL RTLAB
# steps of 3MW
#################################################################
...
def plot_charts_from_csv_mat():
    ...
    #####################################
    # FOLDER LOCATION
    signal_list_folder = '../Modelos/PrimReserveSharingRTLAB/simdata/20220816/'
    ...
    ###############################################################
    # FOLDER LOCATION
    data_folder = '../Modelos/PrimReserveSharingRTLAB/simdata/20220816/'
    ...
    data_full_path_01 = '../Modelos/PrimReserveSharingRTLAB/simdata/20220816/PHIL_3MW_FCRN_BTCFLEX.mat'
    ...
    data_full_path_02 = '../Modelos/PrimReserveSharingRTLAB/simdata/20220816/PHIL_3MW_FCRN_BTCGTs.mat'
    ...
    data_full_path_03 = '../Modelos/PrimReserveSharingRTLAB/simdata/20220816/PHIL_3MW_FCRN_GTs.mat'
    ...
    figure_full_path = figure_folder + 'ChFreq_Steps_PowFact_RTLAB.pdf'  # adds the 'eps
    ...
    figsizex = 6
    figsizey = 6
    fig_steps, axs_steps = plt.subplots(4, 2, sharex=True,
                                        figsize=(figsizex, figsizey),
                                        num='Step3MW')
    # stiles for the RTLAB plots
    aux = 'solid'  # (0, (1, 5))  # 'dotted'   # (0, (1, 10))
    cor_rt = [pe.cor_dalt['blue'], pe.cor_dalt['gray'], pe.cor_dalt['red']]
    ...
    cor_pf = [pe.cor_dalt['blue'], pe.cor_dalt['gray'], pe.cor_dalt['red']]
    ...
    # POWER FACTORY
    ###############################################################
    csvfolder = "../Modelos/PrimReserveSharingPowFact/"
    ...
    for simcount in range(2, -1, -1):
        ...
        axs_steps[3, 0].plot(df_PowFact["time"], df_PowFact["pwf"],
                             linestyle=linestyle_pf[simcount],
                             linewidth=linewidth_pf[simcount],
                             color=cor_pf[simcount],
                             label=labeltext_pf[simcount])

        axs_steps[1, 1].plot(df_PowFact["time"], df_PowFact["pgts"],
                             linestyle=linestyle_pf[simcount],
                             linewidth=linewidth_pf[simcount],
                             color=cor_pf[simcount],
                             label=labeltext_pf[simcount])

        axs_steps[1, 0].plot(df_PowFact["time"], df_PowFact["pfixed"],
                             linestyle=linestyle_pf[simcount],
                             linewidth=linewidth_pf[simcount],
                             color=cor_pf[simcount],
                             label=labeltext_pf[simcount])

        axs_steps[2, 0].plot(df_PowFact["time"], df_PowFact["pflex"],
                             linestyle=linestyle_pf[simcount],
                             linewidth=linewidth_pf[simcount],
                             color=cor_pf[simcount],
                             label=labeltext_pf[simcount])

        axs_steps[3, 1].plot(df_PowFact["time"], df_PowFact["pfcc"] + df_PowFact["pelc"],
                             linestyle=linestyle_pf[simcount],
                             linewidth=linewidth_pf[simcount],
                             color=cor_pf[simcount],
                             label=labeltext_pf[simcount])
    ...
    # axis labels
    ##########################################################################
    axs_steps[3, 0].set_xlabel(r'Time (s)')
    axs_steps[3, 1].set_xlabel(r'Time (s)')

    axs_steps[0, 0].set_ylabel(r'Busbar volt. (kV)')
    axs_steps[0, 1].set_ylabel(r'Busbar freq. (Hz)')
    axs_steps[3, 0].set_ylabel(r'WF power (MW)')

    axs_steps[1, 1].set_ylabel(r'GTs elec. power (MW)')
    axs_steps[1, 0].set_ylabel(r'Load power (MW)')
    axs_steps[2, 0].set_ylabel(r'FLX power (MW)')

    axs_steps[3, 1].set_ylabel(r'ELC+FCC pow.(MW)')
    ...
    # axis limits
    ##########################################################################
    axs_steps[3, 0].set_xticks(np.arange(-20, 180, 10))
    axs_steps[3, 0].set_xlim([0, 100])
    ...
    axs_steps[3, 0].set_yticks(np.arange(11, 13, 0.1))
    axs_steps[3, 0].set_ylim([11.6, 11.9])
    axs_steps[3, 0].yaxis.set_major_formatter(FormatStrFormatter('%.1f'))

    axs_steps[1, 1].set_yticks(np.arange(29, 40, 1))
    axs_steps[1, 1].set_ylim([31, 36])
    axs_steps[1, 1].yaxis.set_major_formatter(FormatStrFormatter('%.0f'))

    axs_steps[1, 0].set_yticks(np.arange(35, 45, 1))
    axs_steps[1, 0].set_ylim([36.8, 40.2])
    axs_steps[1, 0].yaxis.set_major_formatter(FormatStrFormatter('%.0f'))

    axs_steps[2, 0].set_yticks(np.arange(3, 7, 0.5))
    axs_steps[2, 0].set_ylim([4.3, 6.2])
    axs_steps[2, 0].yaxis.set_major_formatter(FormatStrFormatter('%.1f'))

    axs_steps[3, 1].set_yticks(np.arange(-3, 4, 1))
    axs_steps[3, 1].set_ylim([-1.5, 2.2])
    axs_steps[3, 1].yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
    ...
    axs_steps[3, 0].annotate(r'g', xy=(0.9, 0.15), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    axs_steps[1, 1].annotate(r'd', xy=(0.9, 0.15), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    axs_steps[1, 0].annotate(r'c', xy=(0.15, 0.15), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    axs_steps[2, 0].annotate(r'e', xy=(0.9, 0.15), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    axs_steps[3, 1].annotate(r'h', xy=(0.9, 0.15), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    axs_steps[2, 1].annotate(r'f', xy=(0.9, 0.15), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    ##########################################################################
    # axis legends
    ##########################################################################
    axs_steps[1, 0].legend(loc='lower right', frameon=False, prop={'size': 8})
