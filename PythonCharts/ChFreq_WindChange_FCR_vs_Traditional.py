...
# Generates figure with Real Time PHIL simulation comparing
#    extended FCR versus fixed droop
# Loss of 11MW of wind... (changed the wind power setpoint wind speed)
#################################################################
...
def plot_charts():
    ...
    #####################################
    # FOLDER LOCATION
    signal_list_folder = '../Modelos/PrimReserveSharingRTLAB/simdata/20230106/'
    ...
    if transient_type == 'Load3':
        data_full_path = '../RTLab/simdata/20221220/RT_Wind_3MW_FCR.mat'  # this file is not copied... lost
    else:
        data_full_path = '../Modelos/PrimReserveSharingRTLAB/simdata/20230106/PHIL_Wind_12_6m_s_FCR.mat'
    ...
    # FILE TWO
    if transient_type == 'Load3':
        data_full_path = '../RTLab/simdata/20221220/PHIL_Wind_3MW_Trad.mat'  # this file is not copied... lost
    else:
        data_full_path = '../Modelos/PrimReserveSharingRTLAB/simdata/20230106/PHIL_Wind_12_6m_s_Trad.mat'
    ...
    figure_full_path = '../Modelos/PrimReserveSharingRTLAB/simdata/20230106/' + 'ChFreq_FCRxFixDrp_Wind_12_6_m_s.pdf'
    ...
    figsizex = 6  # same as in Fig.9 first submission
    figsizey = 4.5  # 5 was in the Fig.9 of first submission

    fig_steps, axs_steps = plt.subplots(3, 2, sharex=True,
                                        figsize=(figsizex, figsizey),
                                        num='FCRxTrad')

    cor = [pe.cor_dalt['blue'], pe.cor_dalt['red']]
    ...
    axs_steps[0, 1].set_ylabel(r'GTs mech.power(MW)')
    axs_steps[1, 0].set_ylabel(r'BTC power (MW)')
    axs_steps[1, 1].set_ylabel(r'FLEX power (MW)')
    axs_steps[2, 0].set_ylabel(r'Load minus WF(MW)')
    axs_steps[2, 1].set_ylabel(r'Second.control.(MW)')

    ##########################################################################
    # axis limits
    ##########################################################################
    axs_steps[2, 0].set_xticks(np.arange(-20, 180, 10))
    axs_steps[2, 0].set_xlim([-0, 90])  # 120

    axs_steps[0, 0].set_yticks(np.arange(40, 60, 1))
    axs_steps[0, 0].set_ylim([47, 50.25])

    axs_steps[0, 1].set_yticks(np.arange(30, 46, 2))
    axs_steps[0, 1].set_ylim([32, 40])

    axs_steps[1, 0].set_yticks(np.arange(-1, 3, 0.5))
    axs_steps[1, 0].set_ylim([-0.1, 1.6])

    axs_steps[1, 1].set_yticks(np.arange(1, 10, 0.5))
    axs_steps[1, 1].set_ylim([4.4, 6.1])

    axs_steps[2, 0].set_yticks(np.arange(10, 40, 2))
    axs_steps[2, 0].set_ylim([24, 36])

    axs_steps[2, 1].set_yticks(np.arange(-12, 20, 4))
    axs_steps[2, 1].set_ylim([-13, 9])
    ...
    ##########################################################################
    # GT Power
    axs_steps[0, 1].annotate(r'Tradit.droop+sec.power',
    ...
    axs_steps[0, 1].annotate(r'FCR$_{\textrm{D}}$+sec.power',
                             fontsize=9,
    ...
    axs_steps[0, 1].annotate(r'Secondary power references (dashed)',
                             fontsize=9,
    ...
    ##########################
    # BTC power
    axs_steps[1, 0].annotate(r'FCR$_{\textrm{N}}$',
                             fontsize=9,
    ...
    axs_steps[1, 0].annotate(r'Traditional droop',
                             fontsize=9,
    ...
    axs_steps[1, 0].annotate(r'c', xy=(0.05, 0.3), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    axs_steps[2, 0].annotate(r'e', xy=(0.05, 0.3), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    axs_steps[0, 1].annotate(r'b', xy=(0.05, 0.3), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    axs_steps[1, 1].annotate(r'd', xy=(0.05, 0.3), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))

    axs_steps[2, 1].annotate(r'f', xy=(0.05, 0.2), xycoords='axes fraction',
                             bbox=dict(boxstyle='circle', fc=corlegenda))
    ...
    axs_steps[2, 0].legend(loc='best', frameon=False, prop={'size': 9})
    axs_steps[2, 1].legend(loc='best', frameon=False, prop={'size': 9})
