 ...
 *
 */
void pi_std_antiwindup_holdUpDown_Outputs_wrapper(real_T *out,
			real_T *sOutAtMax,
			real_T *sOutAtMin,
			real_T *sIAtMax,
			real_T *sIAtMin,
 ...
 *
 */
void pi_std_antiwindup_holdUpDown_Update_wrapper(const real_T *error,
			const real_T *cPoff,
			const real_T *cIblockUp,
			const real_T *cIhold,
			const real_T *cIblockDown,
...
/* Block integrator up or down */
if((*cIblockUp > 0) && (*error > 0)){
    Ierror = 0;
}
else if((*cIblockDown > 0) && (*error < 0)){
    Ierror = 0;
}
else{
    Ierror = *error;
